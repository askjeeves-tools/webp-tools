...
test("homepage has SEO meta tags and structured data", async ({ page }) => {
	...
	await expect(expander.locator("summary")).toHaveText("Supported WEBP conversions");
	await expect(expander.locator("li")).toHaveCount(7);
	expect(await page.content()).toContain("WebP → PNG");
	...
	await expect(
		seoExpanders.filter({
			has: page.locator("summary", { hasText: "Supported WebP conversions" }),
		}),
	).toBeVisible();
	await expect(
...
test("static SEO files are served", async ({ request }) => {
	...
	expect(robotsText).toContain("Sitemap: https://webp.askjeeves.cc/sitemap-index.xml");
